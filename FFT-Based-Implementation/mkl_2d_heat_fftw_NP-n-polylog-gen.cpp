 ...
 * For running -->
 * ./mkl_2d_heat_fftw_NP-n-polylog-gen 8000 8000 8000 64 32768 68 0
 * Success: memory allocated!
 * N1 = 8000, N2 = 8000, T = 8000, T_BASE = 64, V_BASE = 32768, numThreads = 68, time = 11.844
 *          
 */
...
void create_heat_stencil( double *stencil_real, int N1, int N2 )
{
    int N1N2 = N1 * N2;
    ...
    stencil_real[ 0 ] = ( -2.0 ) * 0.125 * 2.0 + 1;
    stencil_real[ 1 ] = 0.125;    
    stencil_real[ N2 ] = 0.125;    
    stencil_real[ N2 - 1 ] = 0.125;    
    stencil_real[ N1N2 - N2 ] = 0.125;        
}
...
int find_stencil_index( int N1, int N2, int T )
{
    ...
    if ( got == stencil_map.end( ) )
      {            
        ...
        if  ( got == stencil_map.end( ) )
           {        
	         create_heat_stencil( stencil_real, N1, N2 );
        ...
        if ( r & 1 )
           {
	        ...
	        for ( i = 1; i < N1 - 1; i++ ) 
	          {
				...
				#pragma vector always	            
    	        for ( j = 1; j < N2 - 1; j++ ) 
                   tmp___real[ j ] = 0.125 * ( output___real[ j - 1 ] + output___real[ j + 1 ] + output___real[ j - N2 ] + output___real[ j + N2 ] + 4 * output___real[ j ] );

                tmp___real[ 0 ] = 0.125 * ( output___real[ 1 ] + output___real[ - N2 ] + output___real[ N2 ] + 4 * output___real[ 0 ] );                   

                output___real += ( N2 - 1 );
                tmp___real[ N2 - 1 ] = 0.125 * ( output___real[ -1 ] + output___real[ -N2 ] + output___real[ N2 ] + 4 * output___real[ 0 ] );                
              }       
			...
			#pragma vector always	            	        
   	        for ( j = 1; j < N2 - 1; j++ ) 
    	       {
                 tmp__real[ j ] = 0.125 * ( output__real[ j - 1 ] + output__real[ j + 1 ] + output__real[ j + N2 ] + 4 * output__real[ j ] );
                 
    	         int k = ( N1N2 - N2 ) + j;
                 tmp__real[ k ] = 0.125 * ( output__real[ k - 1 ] + output__real[ k + 1 ] + output__real[ k - N2 ] + 4 * output__real[ k ] );                 
               }  

            tmp__real[ 0 ] = 0.125 * ( output__real[ 1 ] + output__real[ N2 ] + 4 * output__real[ 0 ] );
            tmp__real[ N2 - 1 ] = 0.125 * ( output__real[ N2 - 2 ] + output__real[ N2 + N2 - 1 ] + 4 * output__real[ N2 - 1 ] );
            tmp__real[ N1N2 - N2 ] = 0.125 * ( output__real[ N1N2 - N2 + 1 ] + output__real[ N1N2 - N2 - N2 ] + 4 * output__real[ N1N2 - N2 ] );
            tmp__real[ N1N2 - 1 ] = 0.125 * ( output__real[ N1N2 - 2 ] + output__real[ N1N2 - N2 - 1 ] + 4 * output__real[ N1N2 - 1 ] );
           }
        else
           {
            int i, j, l;
	        ...
	        for ( i = 1; i < N1 - 1; i++ ) 
	          {
				...
				#pragma vector always	            
    	        for ( j = 1; j < N2 - 1; j++ ) 
                   output___real[ j ] = 0.125 * ( tmp___real[ j - 1 ] + tmp___real[ j + 1 ] + tmp___real[ j - N2 ] + tmp___real[ j + N2 ] + 4 * tmp___real[ j ] );

                output___real[ 0 ] = 0.125 * ( tmp___real[ 1 ] + tmp___real[ - N2 ] + tmp___real[ N2 ] + 4 * tmp___real[ 0 ] );                   

                tmp___real += ( N2 - 1 );
                output___real[ N2 - 1 ] = 0.125 * ( tmp___real[ -1 ] + tmp___real[ -N2 ] + tmp___real[ N2 ] + 4 * tmp___real[ 0 ] );                
              }       
			...
			#pragma vector always	            	        
   	        for ( j = 1; j < N2 - 1; j++ ) 
    	       {
                 output__real[ j ] = 0.125 * ( tmp__real[ j - 1 ] + tmp__real[ j + 1 ] + tmp__real[ j + N2 ] + 4 * tmp__real[ j ] );
                 
    	         int k = ( N1N2 - N2 ) + j;
                 output__real[ k ] = 0.125 * ( tmp__real[ k - 1 ] + tmp__real[ k + 1 ] + tmp__real[ k - N2 ] + 4 * tmp__real[ k ] );                 
               }  

            output__real[ 0 ] = 0.125 * ( tmp__real[ 1 ] + tmp__real[ N2 ] + 4 * tmp__real[ 0 ] );
            output__real[ N2 - 1 ] = 0.125 * ( tmp__real[ N2 - 2 ] + tmp__real[ N2 + N2 - 1 ] + 4 * tmp__real[ N2 - 1 ] );
            output__real[ N1N2 - N2 ] = 0.125 * ( tmp__real[ N1N2 - N2 + 1 ] + tmp__real[ N1N2 - N2 - N2 ] + 4 * tmp__real[ N1N2 - N2 ] );
            output__real[ N1N2 - 1 ] = 0.125 * ( tmp__real[ N1N2 - 2 ] + tmp__real[ N1N2 - N2 - 1 ] + 4 * tmp__real[ N1N2 - 1 ] );
           }           
       }
...
void compute_aperiodic_heat_stencil( double *input__real, double *output__real, double *tmp__real, int N1, int N2, int T )
{
    ...
    for ( int r = 0; r < T; r++ ) 
      {
        ...
        #pragma omp parallel for
        for ( i = 1, l = N2; i < N1 - 1; i++, l += N2 ) 
	       for ( j = 1; j < N2 - 1; j++ ) 
	          {
	            int k = l + j;
                tmp2_real[ k ] = 0.125 * ( tmp_real[ k - 1 ] + tmp_real[ k + 1 ] + tmp_real[ k - N2 ] + tmp_real[ k + N2 ] + 4.0 * tmp_real[ k ] );
              }  
        ...
        for ( j = 1; j < N2 - 1; j++ ) 
	       {
	         int k = j;
             tmp2_real[ k ] = 0.125 * ( tmp_real[ k - 1 ] + tmp_real[ k + 1 ] + tmp_real[ k + N2 ] + 4.0 * tmp_real[ k ] );
           }  
        ...
        for ( i = 1, l = N2; i < N1 - 1; i++, l += N2 ) 
	       {
	         int k = l;
             tmp2_real[ k ] = 0.125 * ( tmp_real[ k + 1 ] + tmp_real[ k - N2 ] + tmp_real[ k + N2 ] + 4.0 * tmp_real[ k ] );
           }  
        ...
        tmp2_real[ 0 ] = 0.125 * ( tmp_real[ 1 ] + tmp_real[ N2 ] + 4.0 * tmp_real[ 0 ] );
        tmp2_real[ N2 - 1 ] = 0.125 * ( tmp_real[ N2 - 2 ] + tmp_real[ N2 + N2 - 1 ] + 4.0 * tmp_real[ N2 - 1 ] );
        tmp2_real[ N1N2 - N2 ] = 0.125 * ( tmp_real[ N1N2 - N2 + 1 ] + tmp_real[ N1N2 - N2 - N2 ] + 4.0 * tmp_real[ N1N2 - N2 ] );
        tmp2_real[ N1N2 - 1 ] = 0.125 * ( tmp_real[ N1N2 - 2 ] + tmp_real[ N1N2 - N2 - 1 ] + 4.0 * tmp_real[ N1N2 - 1 ] );
...
//        omp_set_nested( 1 );        
    ...
    compute_aperiodic_heat_stencil( input_real, output_real, tmp_real, N1_orig, N2_orig, T_orig );
