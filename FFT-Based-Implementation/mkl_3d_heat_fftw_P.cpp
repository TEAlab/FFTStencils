/**
 For compiling --> 
        ...
        icc -std=gnu++98 -O3 -qopenmp -xhost -ansi-alias -ipo -AVX512 mkl_3d_heat_fftw_P.cpp -o mkl_3d_heat_fftw_P -lm -mkl

 For running -->
 *      ./mkl_3d_heat_fftw_P N T numThreads
 *      Example: ./mkl_3d_heat_fftw_P 100 100000 1      
 */
...
int T, N, N_THREADS;
const int BASE = 1024;
// double a1[1][1][1] /*[MAXN][MAXN][MAXN]*/, a2[1][1][1]/*[MAXN][MAXN][MAXN]*/;
double ***a1, ***a2;
        ...
        for (int i = 0; i < N; ++i) 
            for (int j = 0; j < N; j++)
                ...
                //             + (-2.0*(0.125*2.0) + 1.0)*a1[i][j];
                    ...
                    // a2[i][j][k] = (a1[getIdx(i+1,N)][j][k] - 0 * a1[i][j][k] + a1[getIdx(i-1,N)][j][k])
                    //             + (a1[i][getIdx(j+1,N)][k] - 0 * a1[i][j][k] + a1[i][getIdx(j-1,N)][k])
                    //             + (a1[i][j][getIdx(k+1,N)] - 0 * a1[i][j][k] + a1[i][j][getIdx(k-1,N)]);
                    //             // + a1[i][j][k];
                } 
...
int main(int argc, char *argv[])
{
    ...
    //                             + a1[i][j][k];
    vvvd a(sz_formula, vvd(sz_formula, vd(sz_formula, 0.0)));
    a[0][1][1] = 0.125;
    ...
    // print_cube(rotated_result, "rotated");
