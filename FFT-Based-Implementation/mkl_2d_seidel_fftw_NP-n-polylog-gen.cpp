 ...
 * For compiling --> 
        ...
        icc -std=gnu++98 -O3 -qopenmp -xhost -ansi-alias -ipo -qopt-prefetch=9 -AVX512 mkl_2d_seidel_fftw_NP-n-polylog-gen.cpp -o mkl_2d_seidel_fftw_NP-n-polylog-gen -lm -mkl

 * For running -->
 * ./mkl_2d_seidel_fftw_NP-n-polylog-gen 8000 8000 8000 64 32768 68 0
 * Success: memory allocated!
 * N1 = 8000, N2 = 8000, T = 8000, T_BASE = 64, V_BASE = 32768, numThreads = 68, time = 11.824
 *          
 */
...
void create_seidel_stencil( double *stencil_real, int N1, int N2 )
{
    int N1N2 = N1 * N2;
    ...
    stencil_real[ 0 ] = 0.111;
    stencil_real[ 1 ] = 0.111;    
    stencil_real[ N2 ] = 0.111;    
    stencil_real[ N2 + 1 ] = 0.111;        
    stencil_real[ N2 - 1 ] = 0.111;    
    stencil_real[ ( N2 << 1 ) - 1 ] = 0.111;    
    stencil_real[ N1N2 - N2 ] = 0.111;        
    stencil_real[ N1N2 - N2 + 1 ] = 0.111;            
    stencil_real[ N1N2 - 1 ] = 0.111;
}
...
int find_stencil_index( int N1, int N2, int T )
{
    ...
    if ( got == stencil_map.end( ) )
      {            
        ...
        if  ( got == stencil_map.end( ) )
           {        
	         create_seidel_stencil( stencil_real, N1, N2 );
        ...
        if ( r & 1 )
           {
	        ...
	        for ( i = 1; i < N1 - 1; i++ ) 
	          {
				...
				#pragma vector always	            
    	        for ( j = 1; j < N2 - 1; j++ ) 
                   tmp___real[ j ] = 0.111 * ( output___real[ j - N2 - 1 ] + output___real[ j - N2 ] + output___real[ j - N2 + 1 ]
                                             + output___real[ j - 1 ] + output___real[ j ] + output___real[ j + 1 ]
                                             + output___real[ j + N2 - 1 ] + output___real[ j + N2 ] + output___real[ j + N2 + 1 ] );

                tmp___real[ 0 ] = 0.111 * ( output___real[ - N2 ] + output___real[ - N2 + 1 ]
                                          + output___real[ 0 ] + output___real[ 1 ]
                                          + output___real[ N2 ] + output___real[ N2 + 1 ] );

                output___real += ( N2 - 1 );
                tmp___real[ N2 - 1 ] = 0.111 * ( output___real[ - N2 - 1 ] + output___real[ - N2 ]
                                               + output___real[ - 1 ] + output___real[ 0 ]
                                               + output___real[ N2 - 1 ] + output___real[ N2 ] );
              }       
			...
			#pragma vector always	            	        
   	        for ( j = 1; j < N2 - 1; j++ ) 
    	       {
                 tmp__real[ j ] = 0.111 * ( output__real[ j - 1 ] + output__real[ j ] + output__real[ j + 1 ]
                                          + output__real[ j + N2 - 1 ] + output__real[ j + N2 ] + output__real[ j + N2 + 1 ] );
                                  
    	         int k = ( N1N2 - N2 ) + j;
                 tmp__real[ k ] = 0.111 * ( output__real[ k - N2 - 1 ] + output__real[ k - N2 ] + output__real[ k - N2 + 1 ]
                                          + output__real[ k - 1 ] + output__real[ k ] + output__real[ k + 1 ] );                 
               }  


	        tmp__real[ 0 ] = 0.111 * ( output__real[ 0 ] + output__real[ 1 ] 
                                     + output__real[ N2 ] + output__real[ N2 + 1 ] );                                         
	        tmp__real[ N2 - 1 ] = 0.111 * ( output__real[ N2 - 2 ] + output__real[ N2 - 1 ] 
	                                      + output__real[ ( N2 << 1 ) - 2 ] + output__real[ ( N2 << 1 ) - 1 ] );
	        tmp__real[ N1N2 - N2 ] = 0.111 * ( output__real[ N1N2 - ( N2 << 1 ) ] + output__real[ N1N2 - ( N2 << 1 ) + 1 ] 
	                                         + output__real[ N1N2 - N2 ] + output__real[ N1N2 - N2 + 1 ] );
	        tmp__real[ N1N2 - 1 ] = 0.111 * ( output__real[ N1N2 - N2 - 2 ] + output__real[ N1N2 - N2 - 1 ] 
	                                        + output__real[ N1N2 - 2 ] + output__real[ N1N2 - 1 ] );
           }
        else
           {
            int i, j;
	        ...
	        for ( i = 1; i < N1 - 1; i++ ) 
	          {
				...
				#pragma vector always	            
    	        for ( j = 1; j < N2 - 1; j++ ) 
                   output___real[ j ] = 0.111 * ( tmp___real[ j - N2 - 1 ] + tmp___real[ j - N2 ] + tmp___real[ j - N2 + 1 ]
                                                + tmp___real[ j - 1 ] + tmp___real[ j ] + tmp___real[ j + 1 ]
                                                + tmp___real[ j + N2 - 1 ] + tmp___real[ j + N2 ] + tmp___real[ j + N2 + 1 ] );

                output___real[ 0 ] = 0.111 * ( tmp___real[ - N2 ] + tmp___real[ - N2 + 1 ]
                                             + tmp___real[ 0 ] + tmp___real[ 1 ]
                                             + tmp___real[ N2 ] + tmp___real[ N2 + 1 ] );

                tmp___real += ( N2 - 1 );
                output___real[ N2 - 1 ] = 0.111 * ( tmp___real[ - N2 - 1 ] + tmp___real[ - N2 ]
                                                  + tmp___real[ - 1 ] + tmp___real[ 0 ]
                                                  + tmp___real[ N2 - 1 ] + tmp___real[ N2 ] );
              }       
			...
			#pragma vector always	            	        
   	        for ( j = 1; j < N2 - 1; j++ ) 
    	       {
                 output__real[ j ] = 0.111 * ( tmp__real[ j - 1 ] + tmp__real[ j ] + tmp__real[ j + 1 ]
                                             + tmp__real[ j + N2 - 1 ] + tmp__real[ j + N2 ] + tmp__real[ j + N2 + 1 ] );
                                  
    	         int k = ( N1N2 - N2 ) + j;
                 output__real[ k ] = 0.111 * ( tmp__real[ k - N2 - 1 ] + tmp__real[ k - N2 ] + tmp__real[ k - N2 + 1 ]
                                             + tmp__real[ k - 1 ] + tmp__real[ k ] + tmp__real[ k + 1 ] );                     	           	       
               }  


	        output__real[ 0 ] = 0.111 * ( tmp__real[ 0 ] + tmp__real[ 1 ] 
                                        + tmp__real[ N2 ] + tmp__real[ N2 + 1 ] );                                         
	        output__real[ N2 - 1 ] = 0.111 * ( tmp__real[ N2 - 2 ] + tmp__real[ N2 - 1 ] 
	                                         + tmp__real[ ( N2 << 1 ) - 2 ] + tmp__real[ ( N2 << 1 ) - 1 ] );
	        output__real[ N1N2 - N2 ] = 0.111 * ( tmp__real[ N1N2 - ( N2 << 1 ) ] + tmp__real[ N1N2 - ( N2 << 1 ) + 1 ] 
	                                            + tmp__real[ N1N2 - N2 ] + tmp__real[ N1N2 - N2 + 1 ] );
	        output__real[ N1N2 - 1 ] = 0.111 * ( tmp__real[ N1N2 - N2 - 2 ] + tmp__real[ N1N2 - N2 - 1 ] 
   	                                           + tmp__real[ N1N2 - 2 ] + tmp__real[ N1N2 - 1 ] );
           }           
       }
...
void compute_aperiodic_stencil( double *input__real, double *output__real, double *tmp__real, int N1, int N2, int T, int vertical )
{
    ...
    if ( ( T * max( 4, MIN_NT_RATIO ) ) >= min( N1, N2 ) )
       {
    ...
    if ( !vertical )
       {
	    copy_array_horizontal( input__real, input__real + N1N2, N22T );           
		copy_array_horizontal( input__real + ( N1N2 - N22T ), input__real + ( N1N2 + N22T ), N22T );   
		
		compute_aperiodic_stencil( input__real + N1N2, output__real + N1N2, tmp__real + N1N2, ( T << 2 ), N2, T, 0 );

		copy_array_horizontal( output__real + N1N2, output__real, N2T );   
		copy_array_horizontal( output__real + ( N1N2 + N22T + N2T ), output__real + ( N1N2 - N2T ), N2T );      
	   }
    		
	copy_array_vertical( input__real, input__real + N1N2, N1, N2, ( T << 2 ), ( T << 1 ) );           
	copy_array_vertical( input__real + N2 - ( T << 1 ), input__real + N1N2 + ( T << 1 ), N1, N2, ( T << 2 ), ( T << 1 ) );   
		    
	compute_aperiodic_stencil( input__real + N1N2, output__real + N1N2, tmp__real + N1N2, N1, ( T << 2 ), T, 1 );

	copy_array_vertical( output__real + N1N2 + ( ( T * T ) << 2 ), output__real + N2T, N1 - ( T << 1 ), ( T << 2 ), N2, T );           
	copy_array_vertical( output__real + N1N2 + ( ( T * T ) << 2 ) + ( T << 1 ) + T, output__real + N2T + N2 - T, N1 - ( T << 1 ), ( T << 2 ), N2, T );   	
}


void compute_aperiodic_seidel_stencil( double *input__real, double *output__real, double *tmp__real, int N1, int N2, int T )
{
    ...
    for ( int r = 0; r < T; r++ ) 
      {
        ...
        for ( i = 1; i < N1 - 1; i++ ) 
	       for ( j = 1; j < N2 - 1; j++ ) 
	          {
	            int k = i * N2 + j;
                tmp2_real[ k ] = 0.111 * ( tmp_real[ k - N2 - 1 ] + tmp_real[ k - N2 ] + tmp_real[ k - N2 + 1 ] 
                                         + tmp_real[ k - 1 ] + tmp_real[ k ] + tmp_real[ k + 1 ] 
                                         + tmp_real[ k + N2 - 1 ] + tmp_real[ k + N2 ] + tmp_real[ k + N2 + 1 ] );
              }  
        ...
        for ( j = 1; j < N2 - 1; j++ ) 
	       {
	         int k = j;
             tmp2_real[ k ] = 0.111 * ( tmp_real[ k - 1 ] + tmp_real[ k ] + tmp_real[ k + 1 ] 
                                      + tmp_real[ k + N2 - 1 ] + tmp_real[ k + N2 ] + tmp_real[ k + N2 + 1 ] );
           }  
        ...
        for ( i = 1; i < N1 - 1; i++ ) 
	       {
	         int k = i * N2;
             tmp2_real[ k ] = 0.111 * ( tmp_real[ k - N2 ] + tmp_real[ k - N2 + 1 ] 
                                      + tmp_real[ k ] + tmp_real[ k + 1 ] 
                                      + tmp_real[ k + N2 ] + tmp_real[ k + N2 + 1 ] );
           }  
        ...
        for ( i = 1; i < N1 - 1; i++ ) 
	       {
	         int k = i * N2 + N2 - 1;
             tmp2_real[ k ] = 0.111 * ( tmp_real[ k - N2 - 1 ] + tmp_real[ k - N2 ] 
                                      + tmp_real[ k - 1 ] + tmp_real[ k ] 
                                      + tmp_real[ k + N2 - 1 ] + tmp_real[ k + N2 ] );
           }  

        tmp2_real[ 0 ] = 0.111 * ( tmp_real[ 0 ] + tmp_real[ 1 ] 
                                 + tmp_real[ N2 ] + tmp_real[ N2 + 1 ] );                                         
        tmp2_real[ N2 - 1 ] = 0.111 * ( tmp_real[ N2 - 2 ] + tmp_real[ N2 - 1 ] 
                                      + tmp_real[ ( N2 << 1 ) - 2 ] + tmp_real[ ( N2 << 1 ) - 1 ] );
        tmp2_real[ N1N2 - N2 ] = 0.111 * ( tmp_real[ N1N2 - ( N2 << 1 ) ] + tmp_real[ N1N2 - ( N2 << 1 ) + 1 ] 
                                         + tmp_real[ N1N2 - N2 ] + tmp_real[ N1N2 - N2 + 1 ] );
        tmp2_real[ N1N2 - 1 ] = 0.111 * ( tmp_real[ N1N2 - N2 - 2 ] + tmp_real[ N1N2 - N2 - 1 ] 
                                        + tmp_real[ N1N2 - 2 ] + tmp_real[ N1N2 - 1 ] );
...
//        omp_set_nested( 1 );        
    ...
    compute_aperiodic_seidel_stencil( input_real, output_real, tmp_real, N1_orig, N2_orig, T_orig );
