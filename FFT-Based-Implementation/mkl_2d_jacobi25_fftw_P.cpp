/**
 For compiling --> 
        ...
        icc -std=gnu++98 -O3 -qopenmp -xhost -ansi-alias -ipo -AVX512 mkl_2d_jacobi25_fftw_P.cpp -o mkl_2d_jacobi25_fftw_P -lm -mkl

 For running -->
 *      ./mkl_2d_jacobi25_fftw_P N T numThreads
 *      Example: ./mkl_2d_jacobi25_fftw_P 1000 100000 1      
 */
    ...
    for (int t = 0; t < T; ++t) {
        ...
        for (int i = 0; i < N; ++i) 
            for (int j = 0; j < N; j++){
                ...
                for (int k = -2; k <= 2; k++)
                    for (int l = -2; l <= 2; l++)
                        val += a1[getIdx(i + k, N)][getIdx(j+l, N)];
                a2[i][j] = 0.04 * val;
        } 
...
int main(int argc, char *argv[])
{
    ...
    //                         {0, 0.125, 0}}; 
    vvd formula(sz_formula, vd(sz_formula, 0.04));
